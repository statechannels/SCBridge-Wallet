...
export function packUserOp (op: UserOperationStruct, forSignature = true): string {
  if (forSignature) {
    ...
    return AbiCoder.defaultAbiCoder().encode(
      ...
      [op.sender, op.nonce, op.initCode, op.callData, op.callGasLimit, op.verificationGasLimit, op.preVerificationGas, op.maxFeePerGas, op.maxPriorityFeePerGas, op.paymasterAndData, op.signature]
    )
  }
}
...
export function signUserOp (op: UserOperationStruct, signer: BaseWallet, entryPoint: string, chainId: number): string {
  const message = getUserOpHash(op, entryPoint, chainId)
  const msg1 = Buffer.concat([Buffer.from('\x19Ethereum Signed Message:\n32', 'ascii'), Buffer.from(getBytes(message))])
...
export function fillUserOpDefaults (op: Partial<UserOperationStruct>, defaults = DefaultsForUserOp): UserOperationStruct {
